...
const userVerificationV2 = (user) => Boolean(userIds.find(({ id }) => id === user.id)) &&
    newNames.includes(user.name);
const assignUsers = (names, ids) => names.map(({ name }, index) => ({
    name,
    id: ids[index].id,
}));
const nameUsers = (users, names) => {
    const results = [];
...
const updatedUsers = nameUsers(assignUsers(usersToAssign, userIds), newNames);
